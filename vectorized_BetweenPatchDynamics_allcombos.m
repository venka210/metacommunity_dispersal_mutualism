function dydt = vectorized_BetweenPatchDynamics_allcombos(t,y, c_x0, c_xm, c_xym, c_xy, c_y0, c_ym, c_yxm, c_yx, c_mx, c_mxy, e_x0, e_xy, e_xm, e_xym, e_y0, e_yx, e_ym, e_yxm, ...
    e_mx, e_mxy,n)
...
dydt(3,:) = y(3,:).*((c_mx.*(1-y(3,:))-e_mx).*(1-y(2,:)).*y(1,:) + (c_mx.*(1-y(3,:))-e_mx).*(1-y(1,:)).*y(2,:) + (c_mxy.*(1-y(3,:))-e_mxy).*y(1,:).*y(2,:));
dydt = dydt(:);
end