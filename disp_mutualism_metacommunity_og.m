...
del_x = 0.01; del_y = 0.05; del_m = 8:0.3:24; %I'm only not starting from zero because the computational costs are absurd
...
%     end
    ...
    c_mx =(k_x*del_x+k_m*k_eff*a*(1-q)*del_m(i)*local_dens(end,3))*local_dens(end,1);
    
    lambda = c_mx-c_x;
    options2 = odeset('NonNegative',[1,2,3]);

    [t_syst, frac_occup] = ode45(@(t,y)BetweenPatchDynamics_new(t,y, c_x, c_y, c_m, e_x, e_y, e_m, mu, lambda), tspan_meta, (local_dens(end,:))',options2);
    %frac_occup(end,1) = 0;
