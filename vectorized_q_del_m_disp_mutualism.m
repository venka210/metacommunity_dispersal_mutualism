...
d_m = 0.3; 
a = 2.0;
del_m = 18:0.3:30; %I'm only not starting from zero because the computational costs are absurd
...
[t_patch_no_m,local_dens_no_m] = ode45(@(t,y) vectorized_LocalSpeciesInteraction(t,y,r_x,r_y,alpha_xy,alpha_yx, K_x, K_y, del_x, del_y, Del_col', a, Q_col', d_m, num_combinations), tspan/10, repmat(spp_init,1,num_combinations));
...
[t_syst, frac_occup] = ode45(@(t,y)vectorized_BetweenPatchDynamics(t,y, c_x, c_y, c_m, e_x, e_y, e_m, mu, lambda, num_combinations), tspan_meta, local_dens_3d(:));
